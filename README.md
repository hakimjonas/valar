...
* **🎯 Scala Native Support**: Valar now runs on both JVM and Scala Native platforms, enabling validation in
  resource-constrained environments and native applications.
* **📦 Modular Architecture**: The library is now split into focused modules: valar-core for core validation
  functionality and the new optional valar-munit for enhanced testing utilities.
...
// Automatically derive a Validator for User using the givens above  
...
The main breaking change in v0.4.0 is the **artifact name has changed** from valar to valar-core to support the new
